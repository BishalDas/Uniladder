...
<div class="form-group">
	...
	<div class="col-sm-4">
		<input class="col-md-4 form-control" placeholder="Title" name="title" value="{{ old('title', isset($page) ? $page->title : null) }}"
		 type="text" id="title">
	</div>

	{{-- <label class="col-sm-2 control-label" for="slug">Alias</label>
	<div class="col-sm-4">
		<input class="col-md-4 form-control" placeholder="Alias" name="slug" value="{{ old('slug', isset($page) ? $page->slug : null) }}"
		 type="text" id="slug">
	</div> --}}
</div>
...
<div class="form-group">
	...
	<div class="col-sm-10">
		...
		<label style="margin-right: 10px;font-weight:normal;">
			<input name="position[]" 
			{{ is_array(old('position')) && in_array(1, old('position'))? 'checked="checked"' : isset($page) && is_array( $page->position) && in_array(1,  $page->position) ? 'checked="checked"' : null }}
		...
		<label style="margin-right: 10px;font-weight:normal;">
			<input name="position[]" 
			{{ is_array(old('position')) && in_array(2, old('position'))? 'checked="checked"' : isset($page) && is_array( $page->position) &&  in_array(2,  $page->position) ? 'checked="checked"' : null }}
		...
		<label style="margin-right: 10px;font-weight:normal;">
			<input name="position[]" 
			{{ is_array(old('position')) && in_array(3, old('position'))? 'checked="checked"' : isset($page) && is_array( $page->position) && in_array(3,  $page->position) ? 'checked="checked"' : null }}
...
<div class="form-group">
	...
	<div class="col-sm-10">
		@if(isset($page) && $page->image)
			<img src="{{asset('/uploads/page/'.$page->image)}}" width="200">
			<a href="{{route('pages.deleteimage',$page->id)}}" class="btn btn-danger">Delete this image</a>
		@endif
...
<div class="form-group">
	<div class="col-sm-3 col-sm-offset-2">
		<div class="custom-control custom-checkbox mr-sm-2">
			<input type="checkbox" class="custom-control-input" id="status" name="status" value="1" {{ old( 'status', isset($page) ?
			 $page->status : 1) == 1 ? 'checked=checked' : null }}>
